...
% nodes(:,s1) = nodes(:,s1)*M + eb*(Input-nodes(:,s1)'*M*Input-nodes(:,s1));
% for i=1:siez(s1_Neighbors,1)
%     temp = (Input-nodes(:,s1_Neighbors(i)))'* M * (Input-nodes(:,s1_Neighbors(i)));
%     nodes(:,s1_Neighbors(i) ) = nodes(:,s1_Neighbors(i) ) + en*temp;
% end
%  
% Step 6.
